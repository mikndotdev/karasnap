...
export default function MainTemplate({ children }: { children: ReactNode }) {
  return (
    <>
        <Header brand={{ name: "KaraSnap", href: "/", logo: KaraSnapIcon.src, showTitle: true }} buttons={[ { title: "今すぐ使ってみる", href: "/dashboard" } ]} />
        <div className={"mt-24"}>
      {children}
        </div>
        <p className={"text-center text-sm text-muted-foreground py-4"}>
            JOYSOUNDは株式会社エクシングの登録商標です。<br/>
            DAMは第一興商株式会社の登録商標です。<br/>
            その他記載されている会社名、製品名は各社の商標または登録商標です。<br/>
